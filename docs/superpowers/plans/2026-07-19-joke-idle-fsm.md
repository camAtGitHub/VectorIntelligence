# Joke / Question When Idle FSM — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
>
> **Scope:** **Jokes FSM only** (`joke_idle`). Do **not** re-implement Work Day, ambient, greeting, sensor loops, arbiter, presence, chipper Go, or `llm_chat_once` body.
>
> **Binding source (imported in full below):** `docs/FSM-joke-when-idle-spec.md` — this plan is a **verbatim copy of that spec**, plus Phase 0 discovery, subagent dispatch framing, and path anchors verified against the live tree. If this plan and the source ever diverge on a contract, prefer this plan file (it was generated from the source and stamped with codebase line anchors).

**Goal:** When a known/present person has been at the desk but silent for a long while, Vector occasionally speaks first — a genuinely funny one-liner or a good conversation-starting question — then stays quiet for a long time. Generation (LLM) is fully decoupled from serving (SQLite pop); `tick()` never calls an LLM.

**Architecture:** New plugin `JokeIdleBehavior` + `joke_sources` pipeline under `shared/vector-ai/behaviors/`. Interval-guarded asyncio refill loop started from FastAPI startup in `service.py`. Durable tables on existing `ContinuityStore`. Default OFF.

**Tech stack:** Python 3, FastAPI, `httpx.AsyncClient` via existing `llm_chat_once`, SQLite (`ContinuityStore`), asyncio background task, plain `python test_behaviors.py` runner.

**Spec import:** `docs/FSM-joke-when-idle-spec.md` (full text retained in this file from §1 onward).
...
1. **One TASK = one subagent (or one sequential worker).** Tasks touch disjoint files by design; do not merge TASK-01..09 into a single mega-agent.
2. **Strict build order:** TASK-01 → TASK-02 → TASK-03 → TASK-04 → TASK-05 → TASK-06 → TASK-07 → TASK-08 → TASK-09. (04 before 05 because both edit `joke_sources.py`.)
3. **Subagent brief must include:** the full TASK packet from §8 below for that task ID; the Project Bootstrap (§4); the Contracts (§5) if the task touches types; **Allowed APIs** from Phase 0; and the anti-patterns for that task.
4. **Subagent reporting contract (mandatory):** sources consulted, concrete signatures/paths changed, verification commands run + output, confidence + gaps.
5. **Do not invent APIs.** Only use symbols listed under Phase 0 Allowed APIs or binding contracts in this document.
6. **Path root:** all paths in TASK packets that say `behaviors/…` or `service.py` mean under `shared/vector-ai/` (repo layout: `VectorIntelligence/shared/vector-ai/`).
7. **Isolation:** prefer worktree isolation when parallelizing independent *verification* greps; implementation tasks that share nothing *except* build order should still run sequentially.
8. After each TASK: run that task's acceptance checks (or the growing test file). Do not proceed on red.

### Subagent prompt template (copy per task)

```
You are implementing TASK-NN of the Joke Idle FSM plan.

READ FIRST (only these):
- docs/superpowers/plans/2026-07-19-joke-idle-fsm.md — sections: Project Bootstrap (§4), Contracts (§5 if needed), Phase 0 Allowed APIs, and the full TASK-NN packet in §8.
- The "Bootstrap Context" files named in TASK-NN only.

WORKDIR: VectorIntelligence/shared/vector-ai/

DO:
- Implement exactly the Interface Contract in TASK-NN.
- Match existing style (config loaders, ContinuityStore lock+conn, WorkDayBehavior shape).
- Keep tick() free of network/LLM if you touch the FSM.

MUST NOT:
- Touch files outside TASK-NN's "Files to Create / Modify".
- Modify llm_chat_once body, arbiter, presence, workday.py, or chipper Go.
- Invent embeddings clients or cron/wall-clock schedules.

VERIFY: run the Acceptance Criteria checks for TASK-NN (unit tests if present; import checks otherwise).

REPORT: sources, diffs summary, verification output, confidence, gaps.
```

### Shared-file ownership (no two tasks edit the same file)

| File under `shared/vector-ai/` | Owner task |
|--------------------------------|------------|
| `behaviors/config.py` | TASK-01 only |
| `behaviors/continuity.py` | TASK-02 only |
| `behaviors/joke_seeds.txt` | TASK-03 only (CREATE) |
| `behaviors/joke_sources.py` | TASK-04 then TASK-05 (sequential) |
| `behaviors/joke_idle.py` | TASK-06 only (CREATE) |
| `behaviors/runtime.py` | TASK-07 only |
| `service.py` + `env-default` | TASK-08 only |
| `test_behaviors.py` and/or `test_joke_idle.py` | TASK-09 only |
...
|--------|----------------|
| `docs/FSM-joke-when-idle-spec.md` | Full implementation spec (assumptions, module map, contracts, TASK-01..09, risks) — **imported in full below** |
| `shared/vector-ai/behaviors/types.py` | `FaceIdentity`, `PresenceSnapshot`, `TickResult`, `BehaviorContext`, `Behavior` protocol |
| `shared/vector-ai/behaviors/config.py` | `RuntimeConfig`, `WorkdayConfig`, `load_runtime_config`, `load_workday_config`, `_truthy`, `_int`, `resolve_tz` |
| `shared/vector-ai/behaviors/continuity.py` | `ContinuityStore.__init__`, `_lock`, `_conn`, `_init_schema`, `load_workday` / mutate patterns |
| `shared/vector-ai/behaviors/runtime.py` | `BehaviorRuntime.__init__` if-ladder registration; `tick()` invokes `on_speak_allowed` only after arbiter allow; `min_tick_interval` throttle |
| `shared/vector-ai/behaviors/workday.py` | Shape only: `id`, `priority`, `min_tick_interval`, `enabled()`, `tick()`, `on_speak_allowed`, `need_identity` |
| `shared/vector-ai/service.py` | `_behavior_clock_loop` / `_mood_loop` startup pattern; `_continuity` + `BEHAVIOR_RUNTIME` construction (~L183–195); `llm_chat_once` signature (~L329–339) |
| `shared/vector-ai/env-default` | `WORKDAY_*` comment style for `JOKE_*` docs |
| `shared/vector-ai/test_behaviors.py` | `check()` helper, temp-file ContinuityStore, frozen clocks, `__main__` runner |
| Embeddings search under `shared/vector-ai/` | **No embeddings API present** → TASK-05 **must** use lexical novelty fallback (A5) |

### Allowed APIs (use these; do not invent)

| API | Location | Notes |
|-----|----------|--------|
| `Behavior`, `BehaviorContext`, `TickResult`, `PresenceSnapshot`, `FaceIdentity` | `behaviors/types.py` | **READ ONLY** — do not modify |
| `WorkdayConfig`, `load_workday_config`, `RuntimeConfig`, `load_runtime_config`, `_truthy`, `_int`, `resolve_tz` | `behaviors/config.py` | Copy loader style for `JokeConfig` / `load_joke_config` |
| `ContinuityStore`, `self._lock`, `self._conn()`, `_init_schema` | `behaviors/continuity.py` | Add joke tables + accessors only |
| `BehaviorRuntime` | `behaviors/runtime.py` | Add optional `joke_cfg=` kwarg + if-ladder registration |
| `WorkDayBehavior` | `behaviors/workday.py` | **READ ONLY** shape reference |
| `llm_chat_once(messages, *, model=None, temperature=1.0, top_p=0.95, seed=None, timeout=None, max_tokens=None, tag=...)` | `service.py` ~L329 | **READ ONLY** — call from refill only; lazy/injected import |
| `asyncio.create_task(...)` on FastAPI `@app.on_event("startup")` | `service.py` `_mood_loop` / `_behavior_clock_loop` | Copy pattern for refill loop |
| Store instance name | `service.py` `_continuity` | Reuse; do **not** construct a second `ContinuityStore` |
| Runtime construction | `service.py` ~L189 `BEHAVIOR_RUNTIME = BehaviorRuntime(...)` | Add `joke_cfg=JOKE_CFG` keyword |
| Test runner | `python test_behaviors.py` from `shared/vector-ai` | Match `check()` style |

### Verified live anchors (as of plan authoring)

```
service.py:
  L111  asyncio.create_task(_behavior_clock_loop())
  L114  async def _behavior_clock_loop()
  L186  _runtime_cfg = load_runtime_config()
  L187  _workday_cfg = load_workday_config()
  L188  _continuity = ContinuityStore(... / "workday.db")
  L189  BEHAVIOR_RUNTIME = BehaviorRuntime(_runtime_cfg, _workday_cfg, _continuity, quiet_fn=..., voice_ts_fn=...)
  L329  async def llm_chat_once(messages, *, model=None, temperature=1.0, ...)
  L617  async def _mood_loop()
  L625  async def _start_mood_loop() → create_task

runtime.py:
  L20–51  BehaviorRuntime.__init__ registers workday when id in BEHAVIORS_ENABLED and enabled
  L108+   tick() → arbiter.allow → on_speak_allowed only if allowed
  L133–134 min_tick_interval throttle

workday.py:
  L100+   class WorkDayBehavior; min_tick_interval=30.0; on_speak_allowed commits

config.py:
  L52–64  _truthy / _int safe parsers
  L122+   load_workday_config pattern (try/except → disabled defaults)

continuity.py:
  L37+    ContinuityStore; _lock; _conn Row factory; _init_schema CREATE TABLE IF NOT EXISTS
```

### Anti-patterns (global — reject any PR/task that does these)

- Calling LLM / network / camera from `JokeIdleBehavior.tick()` or `pop_line()`.
- Committing cooldown/daily count in `tick()` body (must be `on_speak_allowed` only). Exception: stranger-reject `joke_mark_reject` in body.
- Setting `need_identity=True` every tick or in `audience=="anyone"`.
- Reading joke settings from `ctx.config` (that is workday's config) — use `self.cfg`.
- Wall-clock cron ("refill at 03:00") — Windows sleep kills this; interval guard only.
- Top-level `from service import llm_chat_once` in `joke_sources.py` (import cycle) — lazy or inject.
- Second `ContinuityStore` instance in service wiring.
- New paid embeddings dependency — none exists; use lexical novelty fallback.
- Modifying Work Day / ambient / greeting / sensor / arbiter / presence / `llm_chat_once` body / chipper Go.
- Default-ON behavior or loaders that raise on bad env.
- Using `sqlite3 ':memory:'` with per-call `_conn()` in tests (each connection is a fresh empty DB) — use a temp file.

### File map (joke_idle only)

| Path under `shared/vector-ai/` | Action | Task |
|--------------------------------|--------|------|
| `behaviors/config.py` | MODIFY — `JokeConfig` + `load_joke_config` | 01 |
| `behaviors/continuity.py` | MODIFY — joke tables + accessors | 02 |
| `behaviors/joke_seeds.txt` | CREATE — curated lines | 03 |
| `behaviors/joke_sources.py` | CREATE then MODIFY — serve + refill | 04, 05 |
| `behaviors/joke_idle.py` | CREATE — FSM plugin | 06 |
| `behaviors/runtime.py` | MODIFY — register plugin | 07 |
| `service.py` | MODIFY — load cfg, pass joke_cfg, start loop | 08 |
| `env-default` | MODIFY — document `JOKE_*` | 08 |
| `test_behaviors.py` and/or `test_joke_idle.py` | MODIFY/CREATE — tests | 09 |
| `behaviors/types.py` | READ ONLY | — |
| `behaviors/workday.py` | READ ONLY shape ref | — |
| `behaviors/arbiter.py`, `presence.py` | DO NOT TOUCH | — |

### Module dependency graph (from spec)
...
- [x] Spec task list TASK-01..09 extracted and retained in full below
- [x] Live `llm_chat_once` signature confirmed (model= tiering works)
- [x] Live store name is `_continuity` (not CONTINUITY_STORE) — implementers must use actual name
- [x] No embeddings → lexical novelty required
- [x] Runtime already speech-gates `on_speak_allowed` — joke FSM must not reimplement arbiter
- [x] `ctx.config` is workday_cfg today — joke holds `self.cfg`

### Note on store variable name (spec vs live code)

The source spec examples sometimes say `CONTINUITY_STORE`. **Live code uses `_continuity`.** TASK-08 must pass `store=_continuity` (or the same object already given to `BehaviorRuntime`). Do not invent a new global name unless refactoring for clarity without a second store.

---

# Binding specification (imported from docs/FSM-joke-when-idle-spec.md)

The remainder of this document is the **full** implementation spec, lightly annotated for subagent execution. **Every Interface Contract, constraint, and acceptance criterion below is binding.**
...
### Subagent dispatch (TASK-01)

- **Role:** implementer (single file `config.py`)
- **Read only:** `behaviors/config.py` fully; this TASK-01 packet; Phase 0 Allowed APIs
- **Do not read:** runtime.py, workday.py, service.py
- **Done when:** `load_joke_config({})` works under a one-liner smoke import; acceptance bullets checked
- **Handoff:** expose `JokeConfig` + `load_joke_config` for TASK-02+ (no tests required yet; TASK-09 owns tests)
...
### Subagent dispatch (TASK-02)

- **Role:** implementer (single file `continuity.py`)
- **Read only:** `behaviors/continuity.py` fully; this TASK-02 packet
- **Do not read:** service.py, joke_sources (not yet)
- **Critical:** atomic `joke_queue_pop` under one locked transaction; push dedupes queue+served
- **Done when:** methods exist and can be smoke-tested with a temp-file DB in a throwaway script if desired
- **Handoff:** store API frozen for TASK-04/05/06
...
### Subagent dispatch (TASK-03)

- **Role:** content + file create only
- **No Python logic.** Create `behaviors/joke_seeds.txt` only
- **Done when:** ≥80 jokes + ≥40 questions; format `KIND\tTEXT`; no banned categories
- **Parallelizable:** MAY run in parallel with TASK-01 or TASK-02 (no shared files)
...
### Subagent dispatch (TASK-04)

- **Depends on:** TASK-01, TASK-02, TASK-03 (contracts + seed format)
- **Role:** create `joke_sources.py` serve half only
- **Forbidden:** async refill, LLM calls, embeddings, top-level service import
- **Done when:** `pop_line` / `load_curated` / `parse_json_array` / prompt builders exist and are import-safe
- **Handoff:** pure helpers ready for TASK-05 + FSM
...
### Subagent dispatch (TASK-05)

- **Depends on:** TASK-04 (same file — sequential, do not parallelize with 04)
- **Role:** add `refill_joke_queue` + `_joke_refill_loop` + `novelty` lexical fallback
- **Must inject/mockable:** `llm_chat_once` (lazy import or callable param)
- **LLM-down invariant:** curated alone can fill to `queue_target`
- **Low-watermark:** no-op unless `joke_queue_len() <= refill_low_watermark`
- **Done when:** stubbed LLM path banks vetted lines; garbage batch does not raise
...
### Subagent dispatch (TASK-06)

- **Depends on:** TASK-01, TASK-02, TASK-04
- **Role:** create `joke_idle.py` only
- **Read for shape:** `workday.py` (do not modify); types.py contracts
- **Critical path order:** cap → cooldown → empty → dwell → voice → identity → pop → speech-gated commit
- **Denied-speech:** never commit spoke in body
- **Handoff:** class ready for TASK-07 registration
...
### Subagent dispatch (TASK-07)

- **Depends on:** TASK-06
- **Role:** modify `runtime.py` only — optional `joke_cfg` kwarg + if-ladder
- **Dual gate:** `JOKE_IDLE_ID in enabled` AND `joke_cfg is not None` AND `joke_cfg.enabled`
- **Do not:** change ctx construction, arbiter, or on_speak_allowed invocation
- **Handoff:** constructor signature ready for TASK-08 caller update
...
### Subagent dispatch (TASK-08)

- **Depends on:** TASK-05, TASK-07
- **Role:** wire `service.py` + `env-default` only (~15 lines logic)
- **Live names:** `_continuity`, `BEHAVIOR_RUNTIME`, `load_joke_config`, `JOKE_CFG`
- **Start loop only if** `JOKE_CFG.enabled`; `asyncio.create_task` non-blocking
- **Do not:** second ContinuityStore; do not modify llm_chat_once / mood / clock loops
...
- Startup loop:
  ...
  ```
  **Live code (verified Phase 0):** the store variable is `_continuity` (see `service.py` ~L188).
  Reuse the SAME store instance already passed to `BehaviorRuntime`; do not construct a second `ContinuityStore`.
...
### Subagent dispatch (TASK-09)

- **Depends on:** TASK-01..08 (full stack)
- **Role:** tests only; match `test_behaviors.py` `check()` + temp-file DB style
- **CRITICAL case:** denied-speech (no `on_speak_allowed` → no daily advance)
- **No network / no robot / no real LLM** — inject fake async llm
- **Done when:** all 8 test requirement groups pass via runner
...
## Final Phase (after TASK-09) — Full verification

Run from `shared/vector-ai/`:

```bash
python test_behaviors.py
# if test_joke_idle.py is standalone:
python test_joke_idle.py
```

Grep anti-patterns:

```bash
# tick / pop must not call llm
grep -n "llm_chat_once\|httpx\|AsyncClient\|openai" behaviors/joke_idle.py || true
# pop_line must be sync and not import service at top
head -40 behaviors/joke_sources.py
# default off
grep -n "JOKE_ENABLED" env-default
# registration dual gate
grep -n "joke_idle\|JokeIdle\|joke_cfg" behaviors/runtime.py service.py
```

Manual acceptance (ops):

- `JOKE_ENABLED=0` → no behavior registered, no refill task, import clean.
- `JOKE_ENABLED=1` + `BEHAVIORS_ENABLED=workday,joke_idle` → behavior present; refill task only when enabled.
- Denied speech path: unit test proves daily count does not advance without `on_speak_allowed`.
